...
print("FULL: Line Follow + Detect Saved Templates + Geometry")
...
def detect_and_crop_symbol(frame_rgb):
    ...
    # 1. Heavy Gaussian Blur: Melts away background textures, fingers, and wrinkles
    blur = cv2.GaussianBlur(gray, (7, 7), 0)
   
    # 2. Adaptive Threshold (BlockSize 81, C=8)
    # HUGE BlockSize prevents thick arrows from becoming hollow.
    # It strictly extracts dark marker strokes from lighter backgrounds (like paper).
    ...
    # 3. RETR_LIST: If you hold the paper, the paper's edge won't hide the symbol inside it!
    ...
    for c in contours:
        ...
        if 1000 < area < (W * roi_bottom * 0.6):
            ...
            if x > margin and y > margin and (x + w) < (W - margin) and (y + h) < (roi_bottom - margin):
                ...
                # Aspect Ratio (Reject long table edges/wires)
                if 0.25 < (w / float(h)) < 4.0:
                   
                    # Solidity check (Must be fairly solid, rejects hollow background loops)
    ...
    # Grab the largest valid shape remaining
...
try:
    while True:
        ...
        if contours_line:
            ...
            if cv2.contourArea(c_line) > MIN_LINE_AREA:
                ...
                if M["m00"] != 0:
                    cx = int(M["m10"] / M["m00"])
                    offset = cx - (w // 2)
                    steer = KP_LINE * offset
                    steer = max(-STEER_LIMIT, min(STEER_LIMIT, steer))
                    left_pwm = BASE_PWM - steer
                    right_pwm = BASE_PWM + steer
                    _set_pwm(left_pwm, right_pwm)
                    cv2.circle(disp, (cx, int(h * ROI_START) + roi_line.shape[0] // 2), 6, (0, 255, 0), -1)
                else:
                    _set_pwm(NO_LINE_PWM, NO_LINE_PWM)
            else:
                _set_pwm(NO_LINE_PWM, NO_LINE_PWM)
        else:
            _set_pwm(NO_LINE_PWM, NO_LINE_PWM)

        # ---------------- 2) SYMBOL DETECTION & TEMPLATE MATCHING ----------------
        ...
        cv2.putText(disp, "Top Symbol Searching Area", (5, roi_y - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2)
        ...
        if symbol_box is not None:
            ...
            if det_name is not None:
                ...
                if now - last_print > COOLDOWN:
                    print(f"✅ {display_name} detected (score={det_score:.2f})")
                    last_print = now
            else:
        ...
        if key == ord('t'):
            if crop_gray is None:
                ...
                coast_stop() # Stop motors so it doesn't drive away!
               
                name_input = ""
                # Mini on-screen typing loop
                while True:
                    prompt_disp = disp.copy()
                   
                    # Draw a black text box in the middle of the screen
                    ...
                    if k == 13 or k == 10:  # ENTER Key
                        ...
                        name_input = ""
                        break
                    elif k == 8 or k == 127: # BACKSPACE Key
                ...
                # If they typed a name and hit enter, save it!
                if name_input.strip() != "":
                    idx = 1
